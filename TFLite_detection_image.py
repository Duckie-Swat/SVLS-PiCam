...
parser.add_argument('--edgetpu', help='Use Coral Edge TPU Accelerator to speed up detection',
                    action='store_true')
parser.add_argument('--num_threads', help='Use threading',
                    default=4)
...
if use_TPU:
    ...
    interpreter = Interpreter(model_path=PATH_TO_CKPT, num_threads=num_threads)
...
for image_path in images:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            is_save_ok = True
    print("Detection time of %s is %.2f seconds ---" % (os.path.basename(image_path), time.time() - s))
